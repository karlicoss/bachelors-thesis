...
# Численно долго и тоже решается плохо \todo{Кажется, решается хорошо только для связанных состояний}
\end{easylist}
\end{frame}
...
# Оператор все еще симметрический, и теперь у него есть ненулевые индексы дефекта, можно расширить до самосопряженного
